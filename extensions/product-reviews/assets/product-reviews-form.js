...
(function () {
  ...
  function submitReview(productId) {
    const fetchOptions = {
      ...
      body: JSON.stringify({
        ...
        review: {
          rating: selectElement("#passionfruit-form__rating").value,
          author: selectElement("#passionfruit-form__name").value,
          email: selectElement("#passionfruit-form__email").value,
          title: selectElement("#passionfruit-form__title").value,
          body: selectElement("#passionfruit-form__comment").value,
        },
      }),
    };

    return fetch("/apps/passionfruit/reviews", fetchOptions);
  }
  ...
  function createField({ labelProps, fieldProps = {}, name, parentNode }) {
    ...
    if (labelProps) {
      create({
        ...
        ...labelProps,
        attributes: {
          htmlFor: `passionfruit-form__${name}`,
  ...
  function initWriteReview({ target }) {
    const rootBlock = target.closest(".passionfruit-block[data-product-id]");
    const { productId } = rootBlock.dataset;
    const el = target.parentNode.classList.contains("passionfruit-summary__no-reviews")
      ? target.parentNode
      : target;
    el.parentNode.removeChild(el);

    const container = document.querySelector(".passionfruit-summary");
    ...
    const form = create({
      tag: "form",
      attributes: { className: "passionfruit-summary__form" },
      events: {
        submit: async (event) => {
          event.preventDefault();

          submitButton.textContent = "Sendind 🚀";
          submitButton.classList.add("passionfruit-form__submit--loading");
  ...
  document
    .querySelector(".passionfruit-summary__write-review")
